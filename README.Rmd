...
### Core Functions
`mocapr` contains six core functions and some sample data `mocapr_data`.  
The six core functions are: one import function (`import_captury()`), two functions that project joint-center positions (`project_full_body_to_AP()` and `project_full_body_to_MP()`), and three functions to animate (`animate_global()`, `animate_anatomical()`, and `animate_movement()`).  
