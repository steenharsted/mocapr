...
import_optitrack_csv <- function(filename, frames_pr_second = 50){
  ...
  # Function
  suppressMessages(suppressWarnings(readr::read_csv(
    paste0(filename), skip = 8, col_names = FALSE))) %>%

    #Select the columns of interest
    dplyr::select(X1,      #Frame
                  X42:X44,  #Shoulder - left
                  X49:X51,  #Elbow - left
                  X56:X58,  #Wrist - left
                  X63:X65,  #Hand - left

                  X175:X177, #Shoulder - right
                  X182:X184, #Elbow - right
                  X189:X191, #Wrist - right
                  X196:X198, #Hand - right
