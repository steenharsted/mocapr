...
#' Read an OpenCap TRC file
#'
#' Imports a TRC file exported from OpenCap, cleans the column names,
#' and converts coordinates from meters to millimeters.
#'
#' @param file Path to the .trc file
#' @param .drop_study_cols Logical. If TRUE (default), columns containing "_study" in their names will be removed.
#' These columns are generated by the OpenCap system and typically represent intermediary outputs used during
#' pose estimation rather than finalized joint center positions.
#'
#' @return A tibble with columns:
#' \describe{
#'   \item{frame}{Frame number (integer)}
#'   \item{time_seconds}{Time in seconds (numeric)}
...
#'
import_opencap_trc <- function(file, .drop_study_cols = TRUE) {

  # --- Read metadata ---
  meta_data <- readr::read_lines(file, n_max = 6)

  # Camera / rate info (not used in output but could be stored as attributes)
  meta_data_cam <- suppressWarnings(suppressMessages(readr::read_tsv(I(meta_data), skip = 1, n_max = 1)))

  # --- Column headings ---
  headings1 <- meta_data[4] |>
    stringr::str_split("\\t") |>
    purrr::as_vector() |>
    dplyr::na_if("") |>
    vctrs::vec_fill_missing()

  headings2 <- meta_data[5] |>
    stringr::str_split("\\t") |>
    purrr::as_vector() |>
    stringr::str_remove("[0-9]+$")

  # Clean main headings (shortened names)
  headings1_clean <- headings1 |>
    stringr::str_replace("Frame#", "frame") |>
    stringr::str_replace("Time", "time_seconds") |>
    stringr::str_replace("Shoulder", "S") |>
    stringr::str_replace("Elbow", "E") |>
    stringr::str_replace("Wrist", "W") |>
    stringr::str_replace("Hip", "H") |>
    stringr::str_replace("Knee", "K") |>
    stringr::str_replace("Ankle", "A") |>
    stringr::str_replace("BigToe", "T")

  # Combine headings1 + headings2 for final column names
  headings <- paste0(headings1_clean, headings2)

  # --- Read main data ---
  df <- suppressWarnings(suppressMessages(readr::read_tsv(file, skip = 4, col_types = readr::cols(.default = "d"))))
  names(df) <- headings

  # --- Drop study columns
  if(.drop_study_cols){
    df <- df |>
      dplyr::select(-dplyr::contains("_study"))
  }


  # --- Convert all coordinate columns (meters → millimeters) ---
  df <- df |>
    dplyr::mutate(
      dplyr::across(
        .cols = -c(frame, time_seconds),
        .fns = ~ .x * 1000
      )
    )
